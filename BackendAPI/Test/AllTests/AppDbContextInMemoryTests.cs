...
public class AppDbContextInMemoryTests
{
	...
	public async Task CanSaveAndLoadProduct_InMemory()
	{
		...
		{
			var product = new Product(10m, 100)
			{
				Id = productId,
				Name = "Rozen",
	...
	public async Task CanSaveOrderWithItems_AndCascadeDeletesItems()
	{
		...
		using (var verifyContext = new AppDbContext(options))
		{
			var loadedOrder = await verifyContext.Orders
		...
		{
			var orderToDelete = await deleteContext.Orders
				.Include(o => o.OrderItems)
				.SingleAsync(o => o.Id == orderId);
	...
	public async Task CanSaveKwekerWithProducts_AndQueryByKweker()
	{
		...
		using (var context = new AppDbContext(options))
		{
			...
			var product1 = new Product(2m, 20)
			{
				Id = Guid.NewGuid(),
				Name = "Narcis",
			...
			var product2 = new Product(3m, 30)
			{
				Id = Guid.NewGuid(),
				Name = "Lelie",
		...
		using var verifyContext = new AppDbContext(options);
		var productsForKweker = await verifyContext.Products
			.Where(p => p.KwekerId == kwekerId)
			.OrderBy(p => p.Name)
			.ToListAsync();
