...
function StatsCard({title, icon, data}: { title: string, icon: React.ReactNode, data: string }) {
	return (
		<div className="kweker-stat-card">
			...
			<div className="kweker-stat-row">
				...
				<span className="kweker-stat-row-txt"> This Month</span>
			</div>
		</div>
	);
}
...
function KwekerStats(props: Props) {
	...
	return (
		<section className="kweker-stats">
			<StatsCard
				title="Producten aangeboden"
				icon={<i className="bi bi-bag-fill"></i>}
				data={'1,500 €'}
			/>

			<StatsCard
				title="Producten verkocht"
