...
type Props = {
	order: OrderKwekerOutputDto;
	editMode?: boolean;
	printMode?: boolean;
	onClose?: () => void;
};

function KwekerOrderDetails(props: Props) {
	...
	const updateHandler = useCallback(async () => {
		try {
			...
			else updateState({type: 'error', message: t('order_update_error')});
							...
							<div className={'order-client-ctn'}>
								<div className={'order-client-info-wrapper'}>
									<h4 className="order-client-info-h4">
										<i className="bi bi-person-circle mr-2"></i> {t('client_informations')}
									</h4>
								...
								<div className={'order-client-info-wrapper'}>
									<h4 className="order-client-info-h4">
										<i className="bi bi-geo-alt-fill mr-2"></i> {t('deliver_adres')}
									</h4>
...
export default KwekerOrderDetails;
