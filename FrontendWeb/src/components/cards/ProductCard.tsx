...
export type ProductActions = 'edit' | 'delete' | 'set_pricing';
...
function ProductCard(props: ProductCardProps) {
	...
	const meesterMenu: DropdownItem<ProductActions>[] = [
		{id: 'set_pricing', label: t('set_auction_product_price'), type: 'button', as: 'button', icon: 'bi bi-euro'},

	]

	const handleSelect = useCallback((type: ProductActions) => {
	...
	return (
		<div className="product-card">
			...
			<div className="product-card__content">
				{
					mode == 'meester' && <div className={clsx("product-card__badge !opacity-100 !scale-100 !z-10", product.auctionPlanned ? 'bg-green-500' : 'bg-yellow-400')}>
						{product.auctionPlanned ? t('scheduled_for_auction') : t('no_veiling_planned')}
					</div>
				}
				...
				<div className="product-card__footer">
					<div className="product-card__footer-prices">
						{mode == 'meester' ?
							<>
								<div className="product-card__price-secondary">{formatEur((isKoper ? product.auctionedPrice : product.minimumPrice) ?? 0.00)}</div>
							...
							:
							<>
								<div className="product-card__price">{formatEur((isKoper ? product.auctionedPrice : product.minimumPrice) ?? 0.00)}</div>
							</>
						}
					</div>

					{
						mode != 'selling' && (<CustomDropdown
							...
							items={mode == 'meester' ? meesterMenu : normalMenu}
